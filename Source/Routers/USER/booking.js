...
router.post('/',expressAsyncHandler(async function(req,res){
    ...
    try
    {
        ...
        await Booking.createBooking(bookingId,date,bookingInfo.priceTotal,bookingInfo.showtimeId,user.id);
        ...
        {
            await Ticket.createTicket(item.iSeat,item.iRow,item.iColumn,item.price,bookingId);
        }
        ...
        const time=timeOg.getDate()+"/"+(timeOg.getMonth()+1)+"/"+timeOg.getFullYear()+" - "+timeOg.getHours()+":"+timeOg.getMinutes();
        ...
        {
            htmlListTicket+="<li>ID: "+tickets[i].iSeat+", row "+ tickets[i].iRow+" seat "+tickets[i].iColumn+", price: $"+tickets[i].price+"</li>"
        }
        htmlListTicket+="</ul>";
        ...
        await Email.send(user.Email, subject, "Booking successfull",`<h3>Showtime information</h3><b>Movie:</b> ${showtime[0].NameMovie}<br> <b>Begin at:</b> ${time}<br><b>Cinema:</b> ${showtime[0].NameMvt}<br> <b>Address:</b> ${showtime[0].AddressMvt} <br> <h3>Ticket information</h3> ${htmlListTicket} <b>Total: </b>$${bookingInfo.priceTotal}`);
...
router.get('/getTicketsBooked',expressAsyncHandler(async function(req,res){
...
router.get('/success',expressAsyncHandler(async function(req,res){
